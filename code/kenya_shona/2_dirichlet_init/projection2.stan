...
transformed data {
  vector<lower=0>[K] pop_0_alpha = dirichlet_factor*pop_0_prop_data; // dirichlet parameter
...
parameters {
  ...
  vector<lower=0>[K] gamma_dirichlet; 
...
transformed parameters {
  ...
  // initial population -----------------------------------
  // age proportions (using gamma parameterization of dirichlet:
  props_dirichlet = gamma_dirichlet/sum(gamma_dirichlet);
  
  pop_0_total_true = exp(log_pop_0_total_true_raw*.7 + 7);
  pop_true[1] = pop_0_total_true*props_dirichlet;
...
model {
  ...
  gamma_dirichlet ~ gamma(pop_0_alpha, 1);
  ...
  sigma_fert ~ normal(0, 1);
  sigma_surv ~ normal(0, 1);
}
