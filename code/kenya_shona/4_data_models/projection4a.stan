...
transformed data {
  ...
  vector[K] log_pop_obs_ratios[n_obs];
  
  log_pop_0_ratios_data = log(pop_0_prop_data / pop_0_prop_data[10]);
  
  for (n in 1:n_obs) {
    pop_obs_total[n] = sum(pop_obs[n]); // pop total for each observation
    pop_obs_prop[n] = pop_obs[n] / pop_obs_total[n]; // age proportions for observations
    log_pop_obs_ratios[n] = log(pop_obs_prop[n] / pop_obs_prop[n, 10]); // log ratios
  }
...
model {
  ...
  real pop_true_total;
  vector[K] pop_true_prop;
  vector[K] log_pop_true_ratios;
  ...
  if (t_obs[1] > 0) {
    for (i in 1:n_obs) {
      pop_true_total = sum(pop_true[t_obs[i]]);
      pop_true_prop = pop_true[t_obs[i]] / pop_true_total;
      
      log_pop_true_ratios = log(pop_true_prop/pop_true_prop[10]);
      //print("true", log_pop_true_ratios);
      
      // total population
      pop_obs_total[i] ~ lognormal(log(pop_true_total), sigma_pop);
      
      // population proportions
      log_pop_obs_ratios[i] ~ normal(log_pop_true_ratios, 1); 
      //print("obs", log_pop_obs_ratios[i]);
      //print(sigma_ratios);
