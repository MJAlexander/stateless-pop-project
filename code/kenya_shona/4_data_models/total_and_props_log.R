...
data_list <- list(
  ...
  t_obs = as.array(14),
  n_obs = n_obs,
  pop_obs =  matrix(kenya_pop$n, nrow = 1, ncol = n_ages) %>% pmax(0.001) # rough fix to allow log ratios
)

mod <- cmdstan_model(here("code/kenya_shona/4_data_models/projection4a.stan"))
...
write_rds(to_save, here("output/intermediate/kenya_shona_projection4a.rds"))
